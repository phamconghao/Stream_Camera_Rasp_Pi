...
| 22 | WebRTC | ✅ **Done (6/6 sub-phase)** | Xem breakdown chi tiết 6 sub-phase (22.1-22.6) ở bảng riêng bên dưới — verify bằng Chromium thật (Playwright) đạt `connectionState: connected`, SRTP mã hóa/giải mã thật, multi-client thật (2 viewer độc lập, fan-out đúng, ref-count chính xác). Hướng thay thế HLS (`bridge.sh`) không còn cần thiết nhưng vẫn giữ lại làm phương án dự phòng. CI (`c-cpp.yml`) build cả `camera_app`/`camera_receiver` với `libsrtp2-dev` mỗi lần push. |
...
2. **Test Phase 22 (WebRTC) trên hardware thật** — toàn bộ 6 sub-phase đã verify bằng Chromium thật trong môi trường build (không có camera CSI), nhưng chưa test với luồng video thật từ Pi. Cần: chạy `camera_app` trên Pi, mở `webrtc_test.html` từ máy khác cùng LAN, xác nhận video thật hiển thị được (không chỉ gói RTP giả lập như đã test), và xác nhận `ICE_AGENT_PORT` (40000) không bị firewall/NAT chặn trên mạng thật.
...
4. **Security (Phase 23)** — luồng WebRTC (Phase 22) đã có SRTP/DTLS mã hóa đầy đủ, nhưng luồng RTSP/control-channel legacy (Phase 16-20) vẫn hoàn toàn không mã hóa/không xác thực — `control_channel`/`control_listener_thread` (ai gửi đúng gói UDP tới đúng port đều ép được sender đổi bitrate/tạo keyframe) và chính RTSP server (không có auth, ai biết IP:8554 cũng SETUP/PLAY được). Có thể cân nhắc dùng lại hạ tầng OpenSSL đã có sẵn từ Phase 22 cho việc này thay vì làm mới hoàn toàn.
